...
- 🔴 **Unread Dot** — Apps with unread notifications carry a small dot in the icon's top-right corner. No count, no animation: the rail's tiles are rebuilt whenever the model changes, so a counting badge read the number it was born with as an arrival and pulsed on every rebuild — including one caused by nothing more than moving the pointer between screens.
- 🎯 **Reverse-DNS Icon Fallback** — An application whose id names its vendor in the middle (`org.omarchy.agent`) falls back through the shorter forms — `org.omarchy`, then `omarchy` — since that is frequently the only icon actually installed. Specific names still win: `org.gnome.Nautilus` resolves to `nautilus`, not `gnome`.
- 🌐 **Full Web Apps (PWA) Support** — Automatic domain matching for Chrome/Chromium/Edge/Brave/Vivaldi web apps (Google Maps, Google Contacts, WhatsApp, YouTube, Discord, etc.) with native GTK theme icons.
...
- 🎯 **Highlight Follows the Screen** — Each dock marks the workspace *its own monitor is displaying*, not the one holding keyboard focus. With a dock per screen, highlighting focus would light the same plate everywhere and tell you nothing about what is actually in front of you; a workspace up on another screen keeps a fainter tint instead.
...
| `workspaceScope` | `"all"` | Which workspaces a dock lists — the **Mode** control in the `···` widget. `"monitor"` restricts each dock to the workspaces of its own screen, which with a per-screen block scheme means the external monitor's dock lists its workspaces and the laptop's lists the laptop's. `"all"` lists every workspace on every dock. |
...
| `workspaceStride` | `0` | The offset between one screen's block of workspaces and the next — `10` when the compositor puts workspaces 1-10 on the first screen and 11-20 on the second. It describes the numbering rather than a preference, and applies whichever Mode is chosen: it is what lets a plate be labelled `2` on both screens rather than `2` and `12`, and it is the only reliable way to tell which screen a workspace belongs to. Spanning workspaces. Hyprland cannot put one workspace on two monitors, so multi-monitor setups often pair them by offset — workspace 2 on the main screen and 12 on the second being two halves of one idea. Set this to that offset (usually `10`) and each plate represents the pair: it shows the windows of both halves, clicking it switches every screen at once, and dragging a tile onto it keeps each window on the screen it is already on. `0` gives one plate per workspace. |
